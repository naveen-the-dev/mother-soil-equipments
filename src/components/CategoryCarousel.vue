...
<template><section class="section"><div class="container"><div class="section-heading"><div><p class="eyebrow">{{ t.categories.eyebrow }}</p><h2>{{ t.categories.title }}</h2></div></div><div class="carousel-row"><button class="row-arrow" type="button" :aria-label="t.categories.previous" @click="move(-1)">←</button><div ref="scroller" class="horizontal-scroll row-scroll"><CategoryCard v-for="category in cards" :key="category.id" :category="category" :count="category.count" /></div><button class="row-arrow" type="button" :aria-label="t.categories.next" @click="move(1)">→</button></div></div></section></template>