...
<template><section class="section section-muted"><div class="container"><div class="section-heading"><div><p class="eyebrow">WATCH & LEARN</p><h2>{{ t.videos.title }}</h2><p>{{ t.videos.text }}</p></div></div><div class="carousel-row"><button class="row-arrow" type="button" aria-label="Previous videos" @click="move(-1)">←</button><div ref="scroller" class="horizontal-scroll row-scroll video-scroll"><article v-for="video in videos" :key="video.id" class="video-card clickable" @click="openVideo(video)"><div class="video-thumb youtube-thumb"><img :src="`https://img.youtube.com/vi/${video.youtubeId}/hqdefault.jpg`" :alt="language==='ta'?video.tamil:video.title" loading="lazy"><div class="video-thumb-overlay"></div><div class="play-button">▶</div><span class="youtube-label">YouTube</span></div><div class="video-content"><h3>{{ language==="ta"?video.tamil:video.title }}</h3><p>{{ language==="ta"?video.tamilDescription:video.description }}</p><span>{{ t.videos.watch }} →</span></div></article></div><button class="row-arrow" type="button" aria-label="Next videos" @click="move(1)">→</button></div></div></section></template>